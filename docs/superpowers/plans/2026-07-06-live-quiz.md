...
Expected: container starts, `curl http://localhost:5000/` returns `Live Quiz — under construction`.
...
Run: `cd labs/live-quiz && docker compose up --build`
Then: `curl -s http://localhost:5000/host | grep -o '<option[^>]*>[^<]*' | head -5`
Expected: real topic names from the course's actual item banks (e.g. "weekly:Week 4", not empty).
Then: `docker compose down`.
...
Run: `cd labs/live-quiz && docker compose up --build`, open `http://localhost:5000/host` in a
...
Run: `cd labs/live-quiz && docker compose up --build`, open `http://localhost:5000/` in a second
...
- Tab 1: `http://localhost:5000/host` → pick a topic → "Create game" → note the PIN.
- Tabs 2-4: `http://localhost:5000/` → enter the PIN + nicknames "alice", "bob", "carol" → Join.
...
- Host: open `http://localhost:5000/host`, pick a topic, "Create game" for a PIN.
- Players: open `http://localhost:5000/`, enter the PIN + a pseudonymous nickname (not a real
