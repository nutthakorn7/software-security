...
| Copy-paste of a classmate's evidence | A flag issued per student by a hosted instance is attributable (`seed_flags.py verify`); the local Task 1b flag is the public demo value and proves nothing either way. Actions run links and Code scanning alert pages are repo-specific; viva spot-check the pair |
